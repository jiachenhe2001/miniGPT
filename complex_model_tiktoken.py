...
def unique_in_chunks(data, chunk_size=10000):
    unique_values = set()
...
# Find unique numbers in train_data by processing in chunks
unique_numbers = unique_in_chunks(train_data)
...
learning_rate = 3e-4
max_iteration = 5000
volcab_length = len(unique_numbers)
eval_iters = 200
eval_interval = 500
...
def get_batch(data_set_name):
    ...
    x = torch.stack([torch.from_numpy((data_set[i:i+block_size]).astype(np.int64)) for i in sample_loc])
    y = torch.stack([torch.from_numpy((data_set[i+1:i+1+block_size]).astype(np.int64)) for i in sample_loc])
...
model_file_name = "webgpt_complex"
# Train
...
if os.path.isfile(model_file_name):
    ...
    print("Model loaded.")
    sentence = "Have a warm meal in a cold day means "
    token_ids = enc.encode(sentence)
    initial_input = torch.tensor([token_ids], dtype=torch.long, device=device)
